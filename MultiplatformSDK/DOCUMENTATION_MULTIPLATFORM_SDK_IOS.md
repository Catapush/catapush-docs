...
> **Note:** this is a first draft. The iOS integration is still being polished and a few details (especially around method swizzling and the Swift consumption of `Flow`-based APIs) may change before the 1.0.0 GA release.
...
The Catapush Multiplatform SDK is built with Kotlin Multiplatform: a single shared codebase compiles to native binaries for Android and iOS. On iOS the SDK is shipped as a pre-built **XCFramework** binary target distributed via Swift Package Manager.
...
This section is identical to the legacy iOS pod. Generate a token-based APNs authentication key in your Apple Developer account and register it on the Catapush dashboard:
...
`Catapush.shared.doInit(eventDelegate:mobileServices:)` is the iOS counterpart of `Catapush.init(…)` on Android. The Kotlin function is named `init`, but Kotlin's `init` clashes with Swift's `init` keyword, so SKIE exposes it on Swift as `doInit`.
...
Catapush messages can be delivered through different conversation channels. The SDK exposes the local channels and messages as Kotlin `Flow`s, which Swift consumes via the `AsyncSequence` bridges generated by SKIE.
...
class MyTransformation: CatapushMessageTransformation {
    override func transform(model: CatapushMessageTransformationModel) -> CatapushMessageTransformationModel {
        let original = model.message

        // Example 1 — end-to-end decryption.
        let decryptedBody = MyCrypto.decrypt(original.body, with: MyKeystore.userKey)

        // Example 2 — placeholder substitution.
        let finalBody = decryptedBody
        ...
        let updated = original.doCopy(body: finalBody) // SKIE-generated copy with one field
        return CatapushMessageTransformationModel(message: updated)
    }
}
...
Kotlin's `init` clashes with Swift's `init` keyword. SKIE (the Kotlin/Native interop layer used by the SDK) renames it to `doInit` in the generated Swift bindings.

### How do I consume a Kotlin `Flow` from Swift?

The SDK exposes `Flow`-based APIs (`getMessages`, `getChannelList`, `countMessages`, …). SKIE generates `AsyncSequence` bridges so you can iterate them with `for await … in flow` inside a `Task { … }`. Use `CreateMainScopeKt.createMainScope()` to create a `CoroutineScope` when one is required (for example as the `cachedInScope` parameter of `getMessages(…)`), and cancel the scope in your view controller's `deinit`.
